...
import (
	...
	"modelmatrix_backend/internal/module/modelbuild/model"
...
func (r *BuildRepositoryImpl) Update(build *domain.ModelBuild) error {
	m := r.toModel(build)
	if err := r.db.Model(&model.ModelBuildModel{}).Where("id = ?", build.ID).Updates(m).Error; err != nil {
		return err
	}
	return nil
}

// Delete soft-deletes a model build
