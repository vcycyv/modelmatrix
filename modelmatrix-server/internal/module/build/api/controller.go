...
func (c *BuildController) RegisterRoutes(router *gin.RouterGroup, authMiddleware gin.HandlerFunc) {
	...
	builds.POST("/callback", c.Callback)
...
// Callback godoc
// @Summary Receive training callback from compute service
// @Description Called by compute service when training completes
// @Tags Model Builds
// @Accept json
// @Produce json
// @Param callback body dto.BuildCallbackRequest true "Callback data"
// @Success 200 {object} response.Response
// @Failure 400 {object} response.Response
// @Failure 404 {object} response.Response
// @Router /api/builds/callback [post]
func (c *BuildController) Callback(ctx *gin.Context) {
	var req dto.BuildCallbackRequest
	if err := ctx.ShouldBindJSON(&req); err != nil {
		response.BadRequest(ctx, err.Error())
		return
	}

	if err := c.buildService.HandleCallback(&req); err != nil {
		handleError(ctx, err)
		return
	}

	response.Success(ctx, map[string]string{"message": "Callback processed successfully"})
}
